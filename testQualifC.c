...
int main(int argc, char **argv)
{
   ...
   shmdt(listePilotes);
   int shmid2 = shmget(77, 8 * sizeof(struct Pilote), IPC_CREAT | 0666);
   struct Pilote *listePilotesQualifiés = shmat(shmid, 0, 0);

   //struct Pilote *listePilotesQualifiés = (struct Pilote*) calloc(tailleListePilote,sizeof(struct Pilote));

   for(int i=0;i<8;i++){
    listePilotesQualifiés[i]=listePilotes[i];
   }
   ...
   for(int i = 0; i<3;i++){
    for (int i = 8; i >-1; i--)
    {
            ...
            while (a < 3)
        {
            ...
            int finaleR = 25000 + random;
            totalF += finaleR;
            listePilotesQualifiés[i].tempsTour[a] = finaleR;
            //printf("%d ms        ",finaleR);
            a++;
            ...
            listePilotesQualifiés[i].temps = totalF;
            // usleep(1000000);
	...
	qsort(listePilotesQualifiés, indicePilote +1, sizeof(struct Pilote), compare);
	afficherDonnees(listePilotesQualifiés, 8);
    }
   }
   printf("course terminée");
   shmdt(listePilotesQualifiés);
